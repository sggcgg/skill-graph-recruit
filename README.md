...
| 页面 | 路由 | 功能描述 |
|------|------|---------|
| **首页** | `/` | 数据概览·技能词云·薪资分布·城市热力图·数字递增动画 |
| **岗位搜索** | `/search` | 图谱精准搜索 + RAG 语义搜索双模式·城市/薪资/经验筛选 |
| **技能知识图谱** | `/graph` | D3.js 力导向图·三种布局切换·节点搜索/拖拽/缩放 |
| **AI 对话** | `/chat` | LangGraph Agent 流式对话·多轮上下文·Markdown 实时渲染 |
| **技能匹配分析** | `/match` | Word/PDF 简历解析·技能差距分析·学习路径规划 |
| **数据报表** | `/analytics` | 热门技能排行·技能组合分析·高薪技能分布·AI 洞察 |
| **技能管理** | `/skill-management` | 技能词典增删改查 |
| **用户中心** | `/user-center` | 个人资料·简历管理·收藏岗位·匹配报告历史 |
| **系统监控** | `/monitoring` | 三库健康状态·API 响应时间·服务可用性检测 |

**前端技术亮点：**
- 深色玻璃拟态（Glassmorphism）UI 设计
- Keep-alive 页面缓存 + 路由懒加载
- mammoth 解析 Word + pdfjs 解析 PDF，简历自动提取技能
- SSE 流式接收 Agent 输出，逐字渲染 Markdown
- Pinia 全局状态管理，技能列表预加载避免重复请求
...
│   ├── src/
│   │   ├── views/
│   │   │   ├── Home.vue                # 首页（词云·薪资图·城市分布）
│   │   │   ├── JobSearch.vue           # 双模式岗位搜索
│   │   │   ├── SkillGraph.vue          # D3.js 技能知识图谱
│   │   │   ├── Chat.vue                # AI 流式对话
│   │   │   ├── MatchDashboard.vue      # 简历解析·技能匹配分析
│   │   │   ├── Analytics.vue           # 数据可视化报表
│   │   │   ├── SkillManagement.vue     # 技能词典管理
│   │   │   ├── UserCenter.vue          # 用户中心
│   │   │   └── Monitoring.vue          # 系统监控
│   │   ├── api/                        # Axios 接口层
│   │   ├── components/                 # 公共组件（GlassCard·SkillTag等）
│   │   ├── stores/                     # Pinia 状态管理
